...
func display_damage(value: Big, position: Vector2, is_critical: bool = false):
	...
	number.global_position = Vector2(position.x + randi_range(-50,50), position.y + randi_range(-50,50))
...
func display_gold(value: Big, position: Vector2):
	var number = Label.new()
	number.global_position = Vector2(position.x + randi_range(-10,10), position.y + randi_range(-10,10))
	number.text = value.toAA()
	number.z_index = 5
	number.label_settings = LabelSettings.new()
